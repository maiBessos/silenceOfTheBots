...
namespace GoE.GameLogic.EvolutionaryStrategy
{
    ...
    public abstract class EvaderManagerPolicy<InheritorChromosomeType> : AGoEEvadersPolicy
        where InheritorChromosomeType : IChromosome
    {
        public static InheritorChromosomeType bestChromosome = default(InheritorChromosomeType);
        public abstract void initializeChromosome(IChromosome i, NotifyBasicAlgorithmSuccess notifier);
        public GoE.GameLogic.Utils.DataUnitVec dataInSink { get; protected set; }
        
        protected void updateDataInSink(GameState state)
        {
            // update data that reached to sink
            int r = state.MostUpdatedEvadersLocationRound;
            int rad = EvolutionConstants.radius;
            int transmissionRad = rad - EvolutionConstants.param.r_e;
            foreach (Evader ei in EvolutionConstants.param.A_E)
            {
                // if evader survived, and previously transmitted from area near the sinks, add the data
                if (state.L[state.MostUpdatedEvadersLocationRound][ei].locationType == Location.Type.Node &&
                    state.L[state.MostUpdatedEvadersLocationRound-1][ei].nodeLocation.manDist(EvolutionConstants.targetPoint) >= transmissionRad)
                {
                    dataInSink.Add(state.B_O[state.MostUpdatedEvadersLocationRound][ei]);
                }
            }
        }
    }
